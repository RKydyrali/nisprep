...
class SmartErrorLogEngine:
    ...
    def calculate_next_interval(interval_number: int, ef: float) -> int:
        ...
        return max(1, round(SmartErrorLogEngine.calculate_next_interval(interval_number - 1, ef) * ef))
    ...
    def schedule_review(
        ...
        quality was poor (< 3); otherwise the SM-2 progression applies.
        """
        ...
        if error_count >= 2 and last_quality is not None and last_quality < 3:
            ...
            review_number = max(1, error_count)
            interval = SmartErrorLogEngine.calculate_next_interval(review_number, ef)
